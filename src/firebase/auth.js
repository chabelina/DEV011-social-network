...
export async function loginEmail(email, password) {
  try{
    const user = auth.currentUser; // revisamos si existe una sesión abierta
      if (user) { // En caso de que esté abierta...
        throw new Error('Tiene una sesión abierta: '+user.uid); //Lanza un error en consola indicando el uid de usuario en sesión
      } else { //Si no hay sesión abierta, 
        try { // Intenta ingresar con email y pasword...
          ...
          return user;
        } catch (e) {// Si no logra ingresar, usamos el error 'e' de signInWithEmailAndPassword... 
          ...
          if (e.message.substring(e.message.indexOf('/') + 1, e.message.lastIndexOf(')')) == 'invalid-email') { 
            mensajeErrorLogin = 'Correo incorrecto';
            email.style.border = '3px solid red'; // y pone el cuadro de email en rojo
          } else if (e.message.substring(e.message.indexOf('/') + 1, e.message.lastIndexOf(')')) == 'invalid-login-credentials') {
            mensajeErrorLogin = 'Datos incorrectos';
            email.style.border = '3px solid red'; // y pone el cuadro de email en rojo
            password.style.border = '3px solid red'; // y pone el cuadro de password en rojo
          } else if (e.message.substring(e.message.indexOf('/') + 1, e.message.lastIndexOf(')')) == 'missing-password') {
          ...
          throw new Error(mensajeErrorLogin || e); // se agrega 'e' en caso de que mande un error diferente a los definidos
        }
      }
  }catch(error){ // este sólo está en caso de que alguna de las promesas falle y poder tener el error en consola
...
export async function createUser(email, password) {
  try{
    const response = await createUserWithEmailAndPassword(auth, email, password,); // intenta crear usuario
